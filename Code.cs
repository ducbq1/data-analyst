...
foreach (var fun in namedFunctionsList)
{
  log(fun(x, y));
}

header("We can also accept an arbitrary number of parameters:");
var multiplyN = (int[] numbers) =>
  numbers.Aggregate(1, (a, b) => a * b);
var addN = (int[] numbers) =>
  numbers.Aggregate(0, (a, b) => a + b);
var subtractN = (int[] numbers) =>
  numbers.Aggregate(0, (a, b) => a - b);
var divideN = (int[] numbers) =>
  numbers.Aggregate(1, (a, b) => a / b);
...
header("Return the results as dictionary:");
...
var callByParamCountFn = (int[] values) =>
{
  ...
  var output = values.Lenght switch
  {
    0 => fn0(),
    1 => fnSelf(),
    2 => fnAdd(),
